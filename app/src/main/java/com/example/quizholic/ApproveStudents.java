...
public class ApproveStudents extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private void ApproveUser(ArrayList<Course> coursess) {
        for(Course c: coursess) {
            ...
            userlistReference = FirebaseDatabase.getInstance().getReference(c.CoursesId);
            userlistReference.child(c.UserId).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot snapshot) {
